import React from "react";
import { Github, Linkedin, Mail } from 'lucide-react';
import discord from "../../public/discord.png"

export default function ContactPage() {
  return (
    <div className="min-h-screen px-4 sm:px-6 pt-14 md:pt-24 pb-10">
      <div className="max-w-4xl mx-auto animate-fade-in">

        {/* Header */}
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3">
          Contact 📧
        </h1>
        <p className="text-gray-400 text-base sm:text-lg mb-10 sm:mb-12">
          Feel free to reach out to me. I'm always open to discussing new projects,
          creative ideas, or opportunities.
        </p>

        {/* Contact Info */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 mb-10 sm:mb-12">
          {/* Email */}
          <div className="bg-gray-900/50 p-6 sm:p-8 rounded-lg border border-gray-800 hover:border-blue-500 transition-colors">
            <Mail className="w-8 h-8 sm:w-10 sm:h-10 text-blue-400 mb-3 sm:mb-4" />
            <h3 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2">
              Email
            </h3>
            <a
              href="mailto:bansalyash316@gmail.com"
              className="text-gray-400 hover:text-white break-all transition-colors text-sm sm:text-base"
            >
              bansalyash316@gmail.com
            </a>
          </div>

          {/* GitHub */}
          <div className="bg-gray-900/50 p-6 sm:p-8 rounded-lg border border-gray-800 hover:border-green-500 transition-colors">
            <svg
              className="w-8 h-8 sm:w-10 sm:h-10 text-green-400 mb-3 sm:mb-4"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path d="M12 0C5.373 0 0 5.373 0 12c0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.627-5.373-12-12-12" />
            </svg>
            <h3 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2">
              GitHub
            </h3>
            <a
              href="https://github.com/codeurge123"
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-white transition-colors text-sm sm:text-base break-all"
            >
              github.com/codeurge123
            </a>
          </div>
        </div>

        {/* Social Links */}
        <div className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 p-6 sm:p-8 rounded-lg border border-gray-800">
          <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">
            Let&apos;s Connect
          </h2>

          <div className="flex flex-wrap gap-5">
            <a
              href="https://github.com/codeurge123"
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-white"
            >
              <Github className="w-7 h-7 sm:w-8 sm:h-8" />
            </a>

            <a
              href="https://www.linkedin.com/in/yash-bansal-a67562286"
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-400 hover:text-white"
            >
              <Linkedin className="w-7 h-7 sm:w-8 sm:h-8" />
            </a>

            <a
              href="mailto:bansalyash316@gmail.com"
              className="text-gray-400 hover:text-white"
            >
              <Mail className="w-7 h-7 sm:w-8 sm:h-8" />
            </a>


            <a
              href="https://discord.gg/6BenyqFE"
              target="_blank"
              rel="noopener noreferrer"
              className="group transition-all duration-200 hover:scale-130"
            >
              <img
                src={discord}
                alt="Discord"
                className="w-7 h-7 sm:w-8 sm:h-8 opacity-70 group-hover:opacity-100"
              />
            </a>

          </div>
        </div>

        {/* Footer Note */}
        <div className="mt-10 sm:mt-12 text-center">
          <p className="text-gray-400 mb-2">
            💡 Contact me through email if you have any feedback!
          </p>
          <p className="text-xs sm:text-sm text-gray-500">
            Phone: +91 789580xxxx
          </p>
        </div>

      </div>
    </div>
  );
}
